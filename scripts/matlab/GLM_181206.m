...
if ~isfield(params,'DoFilter'),params.DoFilter=1;end
...
elseif info.flags.lowpass3
        omega_lp=info.flags.omega_lp3;
elseif info.flags.lowpass2
        omega_lp=info.flags.omega_lp2;
elseif info.flags.lowpass1
...
if strcmp(params.type,'events')
    ...
    for j=1:NuniqueEvents
        EDMu(:,j)=sum(EDM(:,info.paradigm.(['Pulse_',num2str(j)])),2);
    end
    EDM=EDMu;
end
